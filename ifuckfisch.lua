...
local Window = Fluent:CreateWindow({
    Title = "Banyu",
    SubTitle = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name .. " | (Banyu hitam)",
    TabWidth = 160,
        ...
        local function disguisechar(char, id)
            task.spawn(function()
                ...
                if hum and desc then
                        ...
                        if shadow then
                            ...
                            if playerShadow and playerShadow ~= lastShadow then
                                for _, part in pairs(playerShadow:GetChildren()) do
                                    if part:IsA("Accessory") or part:IsA("ShirtGraphic") or part:IsA("Shirt") or part:IsA("Pants") then
                                        part:Destroy()
                                    end
                                end
                    ...
                    task.spawn(function()
                        while true do
                            if not workspace:FindFirstChild("Shadows") or not workspace.Shadows:FindFirstChild(localPlayer.Name) then
                                lastShadow = nil
                            end
                            applyShadowDisguise()
                            task.wait(1)
...
local select_player = trade_sec:AddDropdown("select_player", {
    Title = "Select Player",
    Values = player_list,
    Multi = false,
    Default = "",
...
trade_sec:AddToggle("Toggle", {
    ...
    Callback = function(Value)
        ...
        spawn(function()
            while getgenv().config.auto_trade_all_items do task.wait()
...
trade_sec:AddToggle("Toggle", {
    ...
    Callback = function(Value)
        ...
        spawn(function()
            while getgenv().config.auto_accept_trade do task.wait()
...
local jobid = server_sec:AddInput("Input", {
    Title = "Job Id",
    Default = "",
...
misc_sec:AddToggle("Toggle", {
    Title = "Remove Fog",
    Default = getgenv().config.remove_fog or false,
    Callback = function(Value)
...
local rods_equip_amount = rod_multiplier:AddInput("rods_equip_amount", {
...
rod_multiplier:AddToggle("Toggle", {
    ...
    Callback = function(Value)
        ...
        spawn(function()
            while getgenv().config.auto_fail_catch do task.wait()
                if lp:WaitForChild("PlayerGui"):FindFirstChild("reel") then task.wait()
